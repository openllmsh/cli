...
<p align="center"><b>openllm</b> — the OpenLLM CLI.</p>
...
openllm version
openllm mcp            # the unified MCP server (stdio)
```

> Installed from the dashboard (one-click)? That path runs sandboxed and
> skips PATH/completion — run `~/.openllm/bin/openllm setup` once.
...
| `openllm version` | print the version |
...
bun run verify -- --installed         # the `openllm` on your $PATH
```
...
> the install script and `openllm self-update` enforce on download.
