...
const fillCatalogTokens = (doc: unknown, inputs: TLaunchInputs): TJsonObject =>
  mapStrings(doc, (text) =>
    text.replace(/__OPENLLM_BASE_URL__|__OPENLLM_API_KEY__/g, (token) =>
      token === CATALOG_BASE_URL_TOKEN ? `${inputs.apiBase}/v1` : inputs.apiKey,
    ),
  ) as TJsonObject;
...
const overlayDocument = (
  ...
  const source =
    format === "json"
      ? text
      : text.replace(/(?<=args = |args: )\{\{MCP_ARGS\}\}/g, '"{{MCP_ARGS}}"');
  const doc =
    format === "json"
      ? JSON.parse(source)
      : format === "toml"
      ...
      ...(inputs.bare
        ...
        : {
            "mcp.json": JSON.stringify(
              overlayDocument(OVERLAYS.claude.mcp, "json", inputs),
            ),
          }),
    },
...
const planGrok = (inputs: TLaunchInputs): TLaunchPlan => {
  ...
  return {
    files: {
      ...
      "hooks/openllm.json": JSON.stringify(
        overlayDocument(OVERLAYS.grok.hooks, "json", inputs),
      ),
...
const planOpenCode = (inputs: TLaunchInputs): TLaunchPlan => {
  ...
  const overlay = overlayDocument(OVERLAYS.opencode.config, "json", inputs, {
    MODELS: models,
  });
  const user =
    inputs.userConfig === undefined
